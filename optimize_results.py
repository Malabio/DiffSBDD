...
if __name__ == "__main__":
    ...
    parser.add_argument('--mols_to_save', type=str, default='all')
    ...
    for idx, row in evaluation.iterrows():
        ...
        if args.resi_list is not None:
            ...
            residues = utils.get_pocket_from_ligand(pdb_model, ref_mol)
        # Define pocket based on reference ligand
        pocket = model.prepare_pocket(residues, repeats=population_size)
        # Making buffer to save molecules
        # Store molecules in history dataframe 
        buffer_all = pd.DataFrame(columns=['generation', 'objective', 'score', 'fate' 'mol', 'smiles'])

        # For each metric in the objectives list
        for objective in args.objectives:
            if objective == 'qed':
                ...
                ### FUNCTIONS HERE 
                raise ValueError(f"Objective function {objective} not recognized.")
